...
public class UserInfoCache {
    ...
    public static FUser saveGetFree(Context context, int getFree) {
        if (info == null) {
            info = getUserInfo(context);
        }
        info.setIs_getfree(getFree);
        saveUserInfo(context, info);
        return info;
    }
    ...
    public static boolean isGetFree(Context context) {
        if (info == null) {
            info = getUserInfo(context);
        }
        return info.getIs_getfree() == 0;
    }
