package com.nnmedia.read.contact;

/**
 * 公共常量
 */
public interface Common {

    /***福利任务切换MainActivity判断***/
    String SWITCH2_BOOKSTORE = "switch_main_bookstore";         //跳到书城
    String SWITCH2_WELFARE = "switch_main_welfare";             //切换到福利
    String SIGNIN_NEWBIE = "sign_in_newbie";                    //切换到福利
    String WATCH_VIDEO = "watch_video";                         //看视频
    /***福利任务切换MainActivity判断***/
    String VIDEO_BONUSES = "video_bonuses";                      //看视频得金币
    String VIDEO_ADVERT = "video_advert";                        //看视频免广告


    String KEY_CAT_PID = "cat_p_id";
    String KEY_CAT_TOTAL_BOOK = "cat_total_book";

    String KEY_CAT_ID = "cat_id";
    String KEY_CAT_TITLE = "cat_detail_title";

    String KEY_BOOK_ID = "novel_id";
    String KEY_BOOK_DTO = "book_dto";

    String KEY_BOOK_NAME = "book_name";
    String KEY_BOOK_IMG = "book_img";
    String KEY_BOOK_SCORE = "book_score";
    String KEY_BOOK_REVIEW_DTO = "book_review_dto";

    String KEY_KEYWORD = "keyword";

    String KEY_URL = "key_url";
    String KEY_PAY_ID = "key_id";
    String KEY_PAY = "key_pay";

    String SPFKEY_SEARCH_HISTORY = "key_search_history";


    String KEY_TOKEN = "token";

    String KEY_FIRST_LOGIN = "fist_login";

    String KEY_CDKEY = "cdkey";
    String KEY_CASH_ALIPAY = "cash_alipay";
    String KEY_CASH_WEIXIN = "cash_weixin";

    String KEY_H_CDKEY = "h_point_cdkey";
    String KEY_H_ALIPAY = "h_point_alipay";
    String KEY_H_WEIXIN = "h_point_weixin";

    String KEY_CASH_COMBOID = "cash_comboid";
    String KEY_H_COMBOID = "h_point_comboId";
    String KEY_GOLD_COMBOID = "gold_comboid";
    String KEY_POINT_COMBOID = "point_comboid";

    String KEY_ACCOUNT = "account";
    String KEY_PWD = "pwd";

    String EXTRA_KEY_MONEY = "key_money";


    String USER_DTO = "user_dto";
    String KEY_USER_ID = "user_id";
    String KEY_USERNAME = "username";
    String KEY_MOBILE = "mobile";
    String KEY_GENDER = "gender";
    String KEY_DISTRIBUTION = "distribution";
    String KEY_IS_NEW_MAN = "is_new_man";
    String KEY_IS_VIP = "is_vip";
    String KEY_VIP_ENDTIME = "vip_endtime";
    String KEY_IS_VISITOR = "is_visitor";
    String KEY_IS_INVITED = "is_invited";
    String KEY_HTTP_AVATAR = "http_avatar";
    String INVITE_CODE = "invate_code";

    String BANKCARD_NO = "bankcard_no";
    String WITHDRAWAL_DO_TASKS = "withdrawal_do_tasks";

    String SPF_KEY_FIRST_RUN = "is_first_run";

    String SPF_KEY_DOMAIN_LIST = "domain_list";

    String SPF_KEY_IS_NEW_LIST = "is_new_list";

    String SPF_KEY_BADGECOUNT = "launcher_badgeCount";

}