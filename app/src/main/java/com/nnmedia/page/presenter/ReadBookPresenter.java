package com.nnmedia.page.presenter;
...
import com.nnmedia.page.ui.base.BasePresenter;
...
//                .doFinally(this::dismissLoadingDialog)
        ...
        for (int i = 0; i < size; ++i) {
            ...
            RxHttp.postForm(Consts.NOVEL_CONTENT_API)
                    ...
                    .subscribe(data -> {
                        if (isViewAttached()) {
                            //存储数据
                            BookRepository.getInstance().saveChapterInfo(bookId, bookChapter.getTitle(), data.getContent());
                            view.finishChapter();
                        }
                    }, (OnError) error -> {
    ...
    @Override
    public void unlockChapter(AppCompatActivity context, String bookId, String chapterId) {
        checkViewAttached();

        RxHttp.get(Consts.UNLOCK_CHAPTER_API)
                .add(Consts.NOVEL_ID, bookId)
                .add(Consts.CHAPTER_ID, chapterId)
                ...
                .doFinally(this::dismissLoadingDialog)
                .to(RxLife.toMain(context))
                .subscribe(data -> view.unlockChapter(data), (OnError) error -> {
                    view.errorUnlock();
                });
    }
}