package com.nnmedia.comics.core;

import com.nnmedia.comics.manager.ChapterManager;
...
import com.nnmedia.comics.parser.Parser;
import com.nnmedia.comics.parser.SearchIterator;
        ...
        return Observable.create(new Observable.OnSubscribe<Comic>() {
            ...
            public void call(Subscriber<? super Comic> subscriber) {
                try {
                    ...
                    String html = getResponseBody(FrApp.getHttpClient(), request);
                    SearchIterator iterator = (SearchIterator) parser.getSearchIterator(html,page);
...
//                    list.addAll(mongo.QueryComicBase(comic));
                    if (list.isEmpty()) {
                        ...
                        if (request != null) {
                            html = getResponseBody(FrApp.getHttpClient(), request);
                        }
...
//            list.addAll(mongo.QueryComicChapter(source, cid, path));
            ...
            Request request = parser.getImagesRequest(cid, path);
            response = FrApp.getHttpClient().newCall(request).execute();
    ...
    public static Observable<String> loadLazyUrl(final Parser parser, final String url) {
        return Observable.create(new Observable.OnSubscribe<String>() {
            ...
            public void call(Subscriber<? super String> subscriber) {
                ...
                String newUrl = null;
                try {
                    newUrl = parser.parseLazy(getResponseBody(FrApp.getHttpClient(), request), url);
                } catch (Exception e) {
                    e.printStackTrace();
                }
...
//                        .build();
                Request request = new Request.Builder()
                        ...
                        .build();
                try {
                    String jsonString = getResponseBody(FrApp.getHttpClient(), request);
