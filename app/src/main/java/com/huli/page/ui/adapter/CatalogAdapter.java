    ...
    protected void convert(BaseViewHolder helper, TxtChapter item) {
        ...
        if (item.getLink() == null) {
            ...
            if (item.getBookId() != null && FileUtils.isChapterCached(item.getBookId(), item.getTitle())) {
                helper.setText(R.id.category_tv_type, "已下载");
                helper.setTextColor(R.id.category_tv_type, ContextCompat.getColor(getContext(), mPageStyle.getFontColor()));
            } else {
        ...
        if (item.isSelect()) {
            ...
            tv.setTextColor(ContextCompat.getColor(getContext(), mPageStyle.getFontColor()));
        }
