...
public class AttTopSearchAdapter extends BaseQuickAdapter<HotSearchEntity, BaseViewHolder> {
    ...
    protected void convert(@NonNull BaseViewHolder helper, HotSearchEntity item) {
        GlideUtil.loadRoundRect(getContext(), helper.getView(R.id.iv_book_cover), item.getProfileNovel().getHttp_image(), 0);
        helper.setText(R.id.tv_book_name, item.getProfileNovel().getName());
