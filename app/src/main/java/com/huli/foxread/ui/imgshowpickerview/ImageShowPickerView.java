...
//        recyclerView.setLayoutParams(layoutParams);
     ...
     * 最后调用方法显示，必须最后调用
     */
    public void show(int viewWidth, int spacing, boolean includeEdge) {
        MyGridLayoutManager layoutManager = new MyGridLayoutManager(context, oneLineShowNum);
//        layoutManager.setAutoMeasureEnabled(true);
        recyclerView.setLayoutManager(layoutManager);

        if (spacing > 0) {
            recyclerView.addItemDecoration(new GridSpacingItemDecoration(oneLineShowNum, spacing, includeEdge));
            if (includeEdge) {
                mPicSize = (viewWidth - (oneLineShowNum + 1) * spacing) / oneLineShowNum;
            } else {
                mPicSize = (viewWidth - (oneLineShowNum - 1) * spacing) / oneLineShowNum;
            }
        }
