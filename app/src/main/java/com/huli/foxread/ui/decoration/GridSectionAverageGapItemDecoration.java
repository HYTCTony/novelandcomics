package com.huli.foxread.ui.decoration;
...
public class GridSectionAverageGapItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        if (parent.getLayoutManager() instanceof GridLayoutManager && parent.getAdapter() instanceof BaseSectionQuickAdapter) {
            ...
            BaseSectionQuickAdapter<JSectionEntity, BaseViewHolder> adapter = (BaseSectionQuickAdapter) parent.getAdapter();
            if (mAdapter != adapter) {
                setUpWithAdapter(adapter);
            }
            int spanCount = layoutManager.getSpanCount();
            int position = parent.getChildAdapterPosition(view) - mAdapter.getHeaderLayoutCount();
            JSectionEntity entity = adapter.getItem(position);
...
//            Log.w("GridAverageGapItem", "pos=" + position + ",vPos=" + visualPos + "," + outRect.toShortString());
    ...
    private void setUpWithAdapter(BaseSectionQuickAdapter<JSectionEntity, BaseViewHolder> adapter) {
        if (mAdapter != null) {
    ...
    private void markSections() {
        if (mAdapter != null) {
            BaseSectionQuickAdapter<JSectionEntity, BaseViewHolder> adapter = mAdapter;
            mSectionList.clear();
            JSectionEntity sectionEntity = null;
